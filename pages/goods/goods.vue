  ...
  <view>
    ...
    <view class="swiper-box">
      <swiper circular="true" autoplay="true" @change="swiperChange">
        <swiper-item v-for="(swiper, index) in imageList" :key="index">
          <image :src="swiper.url"></image>
        </swiper-item>
      </swiper>
      <view class="indicator"
        >{{ currentSwiper + 1 }}/{{ imageList.length || 0 }}</view
      >
    </view>
    <!-- 标题 价格 -->
    <view class="info-box goods-info">
      <view class="price">￥{{ goodsInfo.priceInfo.price }}</view>
      <view class="title">
        {{ goodsInfo.skuName }}
      </view>
    </view>
    ...
    <view class="info-box comments" id="comments">
      <view class="row">
        <view class="text">商品评价({{ goodsInfo.comments }})</view>
...
export default {
  ...
  methods: {
    // 初始化数据
    async initData(id) {
      const res = await this.$API("/goods/goods_detail", { skuId: id });
      if (res.code === 200) {
